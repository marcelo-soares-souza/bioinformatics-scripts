...
buildCmdLines();
# 6.1. SUB set_trf_options => build trf options command line
# 6.2. SUB set_trap_conf_file => build trap configuration file

# 7. Run TRF/TRAP
runTrfTrap();   # 1 thread ok, more threads (sub 7.1) under construction
# 7.1. SUB runTrfParallel => run TRF in more than 2 threads

# Final stuff
$date = `date`;
print LOG "\n\nPipeline ' pipeline_ssrs_reg_cons.pl ' finished at " . $date;
print "\n\nPipeline ' pipeline_ssrs_reg_cons.pl ' finished :) at " . $date . "\n";
#===================================================================
...
#-------------------------------------------------------------------
# 1. SUB iniStuff => Buils command lines
sub iniStuff
{
	$date = `date`;
	...
	# Log file
	$log_file = "pipeline_ssrs_reg_cons_".$starttime.".log";
	$log_file2 = "temp.log2";
...
sub vars
{
	...
	$log_file          = "pipeline_ssrs_reg_cons.log";
	...
	{
		$conf_file = "pipeline_ssrs_reg_cons.conf";
	}
...
sub configScript {
	...
	(
		...
		GEN1_CODE   => 'Teste_Genome1_code',
		GEN1_FILE   => 'Teste.Genome_name_Version.fasta',
		GEN1_DESC   => 'teste Description of genome',

		PATH_TO_SOFTWARES => '/usr/local/bin/',
		PATH_TO_RESULTS   => '.',

		TRF_MATCH_INDEL_WEIGHT    => '2',
		TRF_MISMATCH_INDEL_WEIGHT => '7',
		TRF_DELTA_INDEL_WEIGHT    => '7',
		TRF_PM                    => '80',
		TRF_PI                    => '10',
		TRF_MIN_SCORE             => '50',
		TRF_MAX_PERIOD            => '500',
		TRF_MASKED_FILE           => 'yes',
		TRF_FLANKING_SEQUENCE     => 'yes',
		TRF_DATA_FILE             => 'yes',
		TRF_SUPPRESS_HTML         => 'no',
		TRF_NGS                   => 'no',
...
sub createDirs
{
	...
	# For trf
	system("mkdir $path_to_results/$starttime/trf/");
	for ($i6=1;$i6<=$config{NUM_SEQS};$i6++)
	{
		system("mkdir $path_to_results/$starttime/trf/gen" . $i6);
	}
	# other ***
	print "done.";
}
...
#-------------------------------------------------------------------
# 6. SUB buildCmdLines => Buils command lines
sub buildCmdLines
{
	# path to software
	...
	# number of threads                   # debug: Temporarily disabled
#   if ( $config{NUM_THREADS} ne ""  )
#   {
#      $threads = $config{NUM_THREADS}; 
#   }

	# number of trf threads
	if ( $config{TRF_THREADS} ne ""  )
	{
		$trf_threads = $config{TRF_THREADS}; 
	}
	...
	# build trf command line for 1 thread
	@cmd_trf = {};

	# build trf command line for >1 threads
	for ($i9=1;$i9<=$config{NUM_SEQS};$i9++)
	{
		for ($i14=1;$i14<=$trf_threads;$i14++)
		{
			$fasta_temp = "temp_".$i14.".fasta";
			$cmd_trf{$i9}{$i14} = $config{TRF}." ".$fasta_temp." ".$trf_options." 2> ".$log_file2." > ".$log_file2;
		}
	}
}
...
sub set_trf_options 
{
	...
	$trf_options = " " . $config{"TRF_MATCH_INDEL_WEIGHT"} . " " . $config{"TRF_MISMATCH_INDEL_WEIGHT"} . " " . $config{"TRF_DELTA_INDEL_WEIGHT"} . " " . $config{"TRF_PM"} . " " . $config{"TRF_PI"} . " " . $config{"TRF_MIN_SCORE"} . " " . $config{"TRF_MAX_PERIOD"} . $trf_m . $trf_f . $trf_d . $trf_h . $trf_ngs;
}
...
sub runTrfParallel
{
	...
	# split genome file and run trf in all temp files at the same time
	...
	# split fasta file in up to $trf_threads temp fastas
	# create temp file names
	@fasta_temp = {};
	for ($i2=1;$i2<=$trf_threads;$i2++)
	{
		if ( $i2 <= $num_fasta )
		{
	...
	# RUN TRF in all fastas
	# run all at same time and save pid numbers
	@childpid = {};
	for ($i13=1;$i13<=$trf_threads;$i13++)
	{
		print ">";
		print "\n \=\> You can follow this trf analysis in another terminal by\n \=\> ' tail -f ".$cmd_cd."/".$log_file2." '\n";
		$childpid{$i1}{$i13} = fork() or exec("$cmd_trf{$i1}{$i13}");
	}
	# wait all pids to finish
	for ($i14=1;$i14<=$trf_threads;$i14++)
	{
		print LOG "\nwaiting PID $childpid{$i1}{$i14} from $cmd_trf{$i1}{$i14}\n";
		waitpid( $childpid{$i1}{$i14},0 );
	}
	print LOG "... genome ".$i1." done.";
	print "\n\n*******************************************************************************************";   # debug
	print "\n*** Remember to delete list and fasta temporary from Parallel trf at the end of scripts ***";     # debug
	print "\n*******************************************************************************************\n";   # debug
	...
	#------------------------------------#
	# NEXT STEPS
	# => CHECK DEMANDED FILES BY THE TRAP
	# => JOIN/RENAME FILES FROM TRF TO TRAP RECOGNIZE THEM AS ONE TRF RESULT
	# => DELETE TEMPORATY FILES
	# => TRANSFORM IN SUBROUTINE?
	# ----------------------------------
